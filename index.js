...
function findPossibleNewPosition(piece, player) {
  if (board[piece.row + player][piece.column + 1] === 0) {
    console.log("line 108", board[piece.row + player][piece.column + 1])
    readyToMove = piece;
    markPossiblePosition(piece, player, 1);
  }

  if (board[piece.row + player][piece.column - 1] === 0) {
    console.log("line 114", board[piece.row + player][piece.column - 1])
    readyToMove = piece;
    markPossiblePosition(piece, player, -1);
  }
}
...
function markPossiblePosition(p, player, direction) {
  // let player = 0
  // let direction = 0
  console.log("markPossible",p, player, direction)
...
function findPieceCaptured(p, player) {
  console.log("find capture", p, "player", player)
  let other = player * -1
  let found = false;
  // player *= -1
  if (
    ...
    markPossiblePosition(newPosition, player, 0);
